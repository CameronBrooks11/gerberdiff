...
def test_apply_format_decimal_passthrough() -> None:
    ...
    assert abs(_apply_format("1.234", spec) - 1.234) < 1e-9
    assert abs(_apply_format("-5.678", spec) - -5.678) < 1e-9
...
def test_apply_format_metric_lz_33() -> None:
    ...
    spec = _spec(UnitType.Millimeter, "LZ", 3, 3)
    assert abs(_apply_format("5000", spec) - 5.0) < 1e-9
    # 10.000 mm → file stores "10000" (leading "0" suppressed)
    assert abs(_apply_format("10000", spec) - 10.0) < 1e-9
    # 0.500 mm → file stores "500" (leading "000" suppressed but none present)
    assert abs(_apply_format("500", spec) - 0.5) < 1e-9
...
def test_apply_format_inch_tz_24() -> None:
    ...
    assert abs(_apply_format("01", spec) - 1.0) < 1e-9
    # 2.0 inch → "02"
    assert abs(_apply_format("02", spec) - 2.0) < 1e-9
    # 0.1234 inch → "001234" (no trailing zeros to suppress)
    assert abs(_apply_format("001234", spec) - 0.1234) < 1e-9
...
def test_apply_format_zero() -> None:
    ...
    assert _apply_format("0", lz) == 0.0
    assert _apply_format("0", tz) == 0.0
